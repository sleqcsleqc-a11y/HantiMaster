    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-xl font-bold text-zinc-900 tracking-tight">Role Management</h3>
            <p className="text-xs text-zinc-500 font-medium">Define system roles and module access levels</p>
          </div>
        </div>
        <div className="flex gap-3">
          <button className="flex items-center gap-2 px-6 py-3 bg-zinc-100 text-zinc-600 rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-zinc-200 transition-all">
            <Copy size={14} />
            Clone Role
          </button>
          <button 
              ...
              >
                <td className="px-8 py-5">
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${role.is_locked ? 'bg-amber-100 text-amber-600' : 'bg-violet-100 text-violet-600'}`}>
                      {role.is_locked ? <Lock size={14} /> : <Shield size={14} />}
                    </div>
                    <span className="text-sm font-bold text-zinc-900 group-hover:text-violet-600 transition-colors">{role.name}</span>
                  </div>
                </td>
                <td className="px-8 py-5">
                  <p className="text-xs text-zinc-500 max-w-xs truncate">{role.description}</p>
                </td>
                <td className="px-8 py-5">
      ...
      <AnimatePresence>
        {isCreateModalOpen && (
          <div className="fixed inset-0 flex items-center justify-center z-[100] p-4">
            ...
            >
              <h3 className="text-xl font-bold text-zinc-900 mb-6">Create New Role</h3>
              <form onSubmit={handleCreateRole} className="space-y-6">
                ...
                <div className="flex gap-4 pt-4">
                  <button 
                    type="button"
                    onClick={() => setIsCreateModalOpen(false)}
                    className="flex-1 px-6 py-3 rounded-2xl border border-zinc-200 text-[10px] font-bold uppercase tracking-widest text-zinc-600"
                  >
                    Cancel
                  </button>
                  <button 
        ...
        {isEditorOpen && selectedRole && (
          <>
            ...
            <motion.div 
              ...
              className="fixed top-0 right-0 bottom-0 w-full max-w-3xl bg-white shadow-2xl z-[70] flex flex-col"
            >
              {/* Header */}
              <div className="p-8 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/50">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-zinc-500 font-medium">Configure granular permissions for this role</p>
                  </div>
                </div>
                  ...
                  {!selectedRole.is_locked && !isEditingRole && (
                    <>
                      <button 
                        ...
                        className="p-2 hover:bg-zinc-100 rounded-xl text-zinc-400 hover:text-violet-600 transition-colors"
                      ...
                      <button 
                        ...
                        className="p-2 hover:bg-zinc-100 rounded-xl text-zinc-400 hover:text-red-600 transition-colors"
                        title="Delete Role"
                      >
                        <Trash2 size={20} />
                      </button>
              ...
              <div className="p-8 border-t border-zinc-100 bg-zinc-50/50 flex justify-end gap-4">
                <button 
                  onClick={() => setIsEditorOpen(false)}
                  className="px-8 py-3 rounded-2xl border border-zinc-200 text-[10px] font-bold uppercase tracking-widest text-zinc-600 hover:bg-zinc-100 transition-all"
                >
                  Close Editor
                </button>
              </div>
