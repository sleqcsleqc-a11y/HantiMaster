    ...
    <div className="space-y-6">
      ...
      <div className="vintsy-card p-6 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <button className="p-2.5 bg-zinc-100 text-zinc-500 rounded-xl hover:bg-zinc-200 transition-all">
            <Filter size={18} />
          </button>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="animate-pulse space-y-4">
            {[1, 2, 3].map(i => <div key={i} className="h-32 bg-zinc-100 rounded-3xl" />)}
          </div>
          ...
          <div className="vintsy-card p-16 text-center">
            ...
            <h4 className="text-lg font-bold text-zinc-900 mb-2">No {activeTab} Requests</h4>
            <p className="text-sm text-zinc-500 max-w-xs mx-auto">All permission requests have been processed for this category.</p>
          </div>
        ) : (
          filteredRequests.map(req => (
            <motion.div 
              ...
              className="vintsy-card p-6 flex flex-col md:flex-row items-center gap-6 group hover:border-violet-200 transition-all cursor-pointer"
              onClick={() => {
                setSelectedRequest(req);
                setIsReviewOpen(true);
              }}
            >
              <div className="w-16 h-16 rounded-3xl bg-orange-50 flex items-center justify-center text-orange-600 border border-orange-100 shadow-sm group-hover:scale-110 transition-transform">
                <Key size={28} />
              </div>
              <div className="flex-1 space-y-2 text-center md:text-left">
                <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
                  <h4 className="text-base font-bold text-zinc-900">{req.user_name}</h4>
                  <span className="hidden md:block w-1 h-1 bg-zinc-300 rounded-full" />
                  <div className="flex items-center justify-center md:justify-start gap-2">
                    <span className="px-2 py-0.5 rounded-lg bg-violet-50 text-[9px] font-bold uppercase tracking-widest text-violet-700 border border-violet-100">
                ...
                <p className="text-xs text-zinc-500 italic font-medium">"{req.justification}"</p>
              ...
              <div className="flex flex-col items-center md:items-end gap-3">
                {activeTab === 'Pending' ? (
                  <div className="flex gap-2">
                    <button className="px-6 py-2.5 rounded-xl border border-zinc-200 text-[10px] font-bold uppercase tracking-widest text-zinc-600 hover:bg-zinc-50 transition-all">
                      Decline
                    </button>
        ...
        {isReviewOpen && selectedRequest && (
          <>
            ...
            <motion.div 
              ...
              className="fixed top-0 right-0 bottom-0 w-full max-w-xl bg-white shadow-2xl z-[70] flex flex-col"
            >
              <div className="p-8 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/50">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-zinc-900 tracking-tight">Review Permission Request</h3>
                    <p className="text-xs text-zinc-500 font-medium">Governance approval workflow</p>
                  </div>
                </div>
                <button 
                  onClick={() => setIsReviewOpen(false)}
                  ...
                  <div className="vintsy-card p-6 space-y-6">
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center py-3 border-b border-zinc-100">
                        ...
                        <span className="text-sm font-bold text-zinc-900">{selectedRequest.user_name}</span>
                      </div>
                      <div className="flex justify-between items-center py-3 border-b border-zinc-100">
                        <span className="text-sm text-zinc-500">Permission</span>
                      ...
                      <div className="flex justify-between items-center py-3 border-b border-zinc-100">
                        ...
                        <span className="text-sm font-bold text-zinc-900">{new Date(selectedRequest.created_at).toLocaleString()}</span>
                      </div>
                      <div className="space-y-2 pt-2">
                        <span className="text-sm text-zinc-500">Expiration Date (Optional)</span>
                        <input 
                          type="date"
                          value={expirationDate}
                          onChange={e => setExpirationDate(e.target.value)}
                          className="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/20"
                        />
                      </div>
                      <div className="space-y-2 pt-2">
                        <span className="text-sm text-zinc-500">Justification</span>
                        <div className="p-4 bg-zinc-50 rounded-2xl border border-zinc-100 text-sm text-zinc-700 italic">
                  ...
                  <div className="vintsy-card p-6 space-y-4">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-3 p-3 bg-zinc-50 rounded-xl border border-zinc-100">
                        ...
                        <span className="text-xs font-medium text-zinc-700">2 Approved Requests</span>
                      </div>
                      <div className="flex items-center gap-3 p-3 bg-zinc-50 rounded-xl border border-zinc-100">
                        <XCircle size={14} className="text-red-500" />
                        <span className="text-xs font-medium text-zinc-700">0 Denied Requests</span>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <>
                    <div className="flex gap-4">
                      <button 
                        ...
                        className="flex-1 px-8 py-4 rounded-2xl border border-zinc-200 text-[10px] font-bold uppercase tracking-widest text-zinc-600 hover:bg-zinc-100 transition-all"
                      >
                        Deny Request
                      </button>
                      <button 
                    ...
                    <button className="w-full px-8 py-3 rounded-2xl border border-zinc-200 text-[10px] font-bold uppercase tracking-widest text-zinc-500 hover:bg-zinc-50 transition-all">
