...
export const AdminDashboard: React.FC = () => {
  ...
  if (loading) return <div className="animate-pulse space-y-8">
    <div className="grid grid-cols-4 gap-4 h-32 bg-zinc-100 dark:bg-zinc-800 rounded-3xl" />
    <div className="grid grid-cols-2 gap-8 h-96 bg-zinc-100 dark:bg-zinc-800 rounded-3xl" />
  </div>;

  const summaryCards = [
    { label: 'Total Users', value: stats?.total_users, icon: Users, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-900/20', border: 'border-blue-100 dark:border-blue-800' },
    { label: 'Suspended Accounts', value: stats?.suspended_users, icon: UserX, color: 'text-red-600', bg: 'bg-red-50 dark:bg-red-900/20', border: 'border-red-100 dark:border-red-800' },
    { label: 'Pending Requests', value: stats?.pending_requests, icon: Key, color: 'text-orange-600', bg: 'bg-orange-50 dark:bg-orange-900/20', border: 'border-orange-100 dark:border-orange-800' },
    { label: 'High-Risk Alerts', value: 4, icon: ShieldAlert, color: 'text-violet-600', bg: 'bg-violet-50 dark:bg-violet-900/20', border: 'border-violet-100 dark:border-violet-800' },
  ];
    ...
    <div className="space-y-8">
        ...
        {summaryCards.map((card, idx) => (
          ...
          >
            ...
            <div className="space-y-1">
              <p className="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest">{card.label}</p>
              <p className="text-3xl font-bold text-zinc-900 dark:text-white tracking-tight">{card.value || 0}</p>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="vintsy-card p-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <span className="px-3 py-1 bg-red-50 dark:bg-red-900/20 text-red-600 text-[10px] font-bold uppercase tracking-widest rounded-full border border-red-100 dark:border-red-800">
            4 Critical Issues
          </span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { title: 'Suspicious Login', user: 'Unknown', risk: 'High', icon: AlertTriangle, color: 'text-red-600', bg: 'bg-red-50 dark:bg-red-900/10' },
            { title: 'Privilege Escalation', user: 'Sarah M.', risk: 'Medium', icon: Shield, color: 'text-orange-600', bg: 'bg-orange-50 dark:bg-orange-900/10' },
            { title: 'Bulk Data Export', user: 'Frank F.', risk: 'Critical', icon: Activity, color: 'text-violet-600', bg: 'bg-violet-50 dark:bg-violet-900/10' },
          ].map((alert, idx) => (
            <div key={idx} className={`${alert.bg} p-6 rounded-3xl border border-white/10 dark:border-black/10 flex items-center gap-4`}>
              <div className={`${alert.color} p-3 rounded-2xl bg-white dark:bg-zinc-900 shadow-sm`}>
                <alert.icon size={20} />
              </div>
              <div>
                <p className="text-sm font-bold text-zinc-900 dark:text-white">{alert.title}</p>
                <p className="text-[10px] text-zinc-500 dark:text-zinc-400 uppercase tracking-widest font-bold">User: {alert.user} • {alert.risk} Risk</p>
              </div>
            </div>
          ))}
        </div>
