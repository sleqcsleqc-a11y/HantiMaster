...
export const TenantDashboard: React.FC<TenantDashboardProps> = ({ setActiveTab }) => {
  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto">
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-zinc-900 dark:text-white tracking-tight">Welcome home, {user?.first_name}</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          <div>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-sm font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">Active Requests</h3>
              <Wrench size={20} className="text-violet-600" />
            </div>
            <div className="space-y-4">
              {requests.length > 0 ? (
                requests.map(req => (
                  <div key={req.id} className="flex items-center gap-3 p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-100 dark:border-zinc-800">
                    <div className={`w-2 h-2 rounded-full ${
                      req.status === 'Completed' ? 'bg-emerald-500' : 'bg-orange-500'
                    }`} />
                    <div className="flex-1 truncate">
                      <p className="text-xs font-bold text-zinc-900 dark:text-white truncate">{req.title}</p>
        ...
        >
          <div>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-sm font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">Lease Info</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 vintsy-card p-8">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-sm font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">Announcements</h3>
            <button className="text-xs font-bold text-violet-600 hover:underline">View All</button>
          </div>
          <div className="space-y-4">
            <div className="p-4 bg-violet-50/50 dark:bg-violet-900/10 border border-violet-100 dark:border-violet-900/30 rounded-2xl">
              <div className="flex items-center gap-3 mb-2">
                <AlertCircle size={16} className="text-violet-600" />
                <h4 className="text-sm font-bold text-zinc-900 dark:text-white">Scheduled Maintenance: Water Main</h4>
              </div>
        ...
        <div className="vintsy-card p-8">
          <h3 className="text-sm font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-500 mb-6">Quick Actions</h3>
          <div className="space-y-2">
            <button 
              onClick={() => setActiveTab('communication')}
              className="w-full flex items-center justify-between p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-2xl border border-zinc-100 dark:border-zinc-800 hover:border-violet-300 dark:hover:border-violet-500 transition-all group"
            >
              <div className="flex items-center gap-3">
                <MessageSquare size={18} className="text-violet-600" />
                <span className="text-xs font-bold text-zinc-900 dark:text-white">Message Manager</span>
              </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-bold text-zinc-900 dark:text-white">Community Rules</span>
              </div>
              <ChevronRight size={16} className="text-zinc-300 group-hover:text-violet-600 transition-colors" />
            </button>
          </div>
