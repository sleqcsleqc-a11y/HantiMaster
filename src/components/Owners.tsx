    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        <div>
          ...
          <p className="text-2xl font-bold text-zinc-900 tracking-tight">Property Owners</p>
        </div>
        <div className="flex items-center gap-4 flex-wrap">
          <div className="relative">
            ...
            <input 
              ...
              className="pl-9 pr-4 py-2 bg-white/50 border border-violet-100 rounded-xl text-xs w-64 text-zinc-900 focus:border-violet-600 focus:ring-4 focus:ring-violet-600/5 outline-none transition-all placeholder:text-zinc-400"
            />
          </div>
          ...
          <select
            ...
            className="px-4 py-2 bg-white/50 border border-violet-100 rounded-xl text-xs font-bold focus:outline-none"
          >
          ...
          <button
            ...
            className="p-2 bg-white/50 border border-violet-100 rounded-xl text-zinc-500 hover:text-violet-600 transition-colors"
          >
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-zinc-900">Add New Owner</h3>
              <button 
            ...
            <form onSubmit={handleAddOwner} className="space-y-4">
              ...
              <div>
                ...
                <select 
                  ...
                  className="w-full px-4 py-2.5 bg-zinc-50 border border-violet-100 rounded-xl text-sm text-zinc-900 focus:border-violet-600 focus:ring-4 focus:ring-violet-600/5 outline-none transition-all h-24"
                >
                  {properties.map(p => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredOwners.map((owner, index) => (
          <motion.div
            ...
            className="vintsy-card p-8 cursor-pointer group hover:border-violet-500 transition-all duration-300"
          >
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h4 className="text-lg font-bold text-zinc-900">{owner.first_name} {owner.last_name}</h4>
                <p className="text-xs text-zinc-500">{owner.email}</p>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4 pt-6 border-t border-violet-50">
              <div>
                <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">Properties</p>
                <div className="flex items-center gap-2">
                  <Building2 size={14} className="text-violet-600" />
                  <span className="text-sm font-bold text-zinc-900">{owner.property_count}</span>
                </div>
              </div>
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-zinc-900">${((owner.total_portfolio_value ?? 0) / 1000000).toFixed(1)}M</span>
                </div>
              </div>
            </div>
